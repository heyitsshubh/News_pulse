...
export const ingestService = {
   ...
   *  1. Create a DB row for the job (status = 'running').
   *  2. Spawn the Python scraper as a detached subprocess passing --job-id.
   *  3. Wire up exit / error handlers to update the DB asynchronously.
   *  4. Return the job record immediately — callers do NOT await completion.
   */
  ...
  /**
   * Look up the current status of an ingest job by its ID.
   * Returns null when the job is not found (caller maps to 404).
   */
